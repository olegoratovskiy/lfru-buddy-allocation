...
Был реализован pool аллокатор, имеющий возможность размещать объекты произвольных размеров
...
позволяет бороться с фрагментацией пула. При этом процедура поиска свободного блока реализована эффективно.
...
размеры размещаемых объектов меньше размера минимального блока, но получилось оптимизировать служебные расходы памяти для такого пула.
